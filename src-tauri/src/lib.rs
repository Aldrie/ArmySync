...
#[tauri::command]
fn parse_effect_file(path: String) -> Result<Vec<Effect>, String> {
    let content =
    ...
    content
        ...
        .map(|(i, line)| {
            ...
            if !VALID_TYPES.contains(&effect_type) {
                return Err(format!("Line {}: unknown effect type '{}'", i + 1, effect_type));
            }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![parse_effect_file, extract_waveform])
